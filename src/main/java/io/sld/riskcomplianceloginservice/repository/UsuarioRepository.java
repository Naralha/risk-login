...
public interface UsuarioRepository extends ReactiveCrudRepository<Usuario, Long>, UsuarioRepositoryInternal {
    @Query("SELECT * FROM usuario entity WHERE entity.empresa_id = :id")
    Flux<Usuario> findByEmpresa(Long id);

    @Query("SELECT * FROM usuario entity WHERE entity.empresa_id IS NULL")
    Flux<Usuario> findAllWhereEmpresaIsNull();
